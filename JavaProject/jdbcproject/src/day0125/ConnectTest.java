...
public class ConnectTest {
	...
	public void connectSwaon() {
		...
		try {
			...
			// rs.next(): 다음데이타로 이동하면서 true반환,더이상 데이타가 없으면 false반환
			while (rs.next()) {
				// db로부터 데이타 가져오기
				...
				System.out.println(num + "\t" + name + "\t" + boseo + "\t" + pay);
			}
...
//			e.printStackTrace();
	...
	public void connectShop() {
		...
		try {
			...
			System.out.println("**회웡님 장바구니 리스트**");
	...
	public void sawonRead() {
		...
		try {
			...
			while (rs.next()) {
				...
				System.out.println(buseo + "\t" + count + "\t" + pay + "원");
