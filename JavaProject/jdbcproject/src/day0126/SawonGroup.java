...
public class SawonGroup {
	...
	public void connectSawonGroup() {
		...
		try {
			...
			System.out.println();
			System.out.println("성별\t인원수\t평균급여\t최저급여");
			System.out.println("----------------------------");
			while (rs.next()) {
	...
	public void personGroup() {
		...
		Statement stmt = null;
		ResultSet rs = null;
		String sql = "select job,count(job) count,round(avg(age),0) age from person group by job";
		conn = db.getOracle();
		try {
			stmt = conn.createStatement();
			rs = stmt.executeQuery(sql);
			System.out.println("job 인원수와 평균나이");
			System.out.println();
			System.out.println("직원\t인원수\t평균나이");
			System.out.println("-----------------");
			while (rs.next()) {
