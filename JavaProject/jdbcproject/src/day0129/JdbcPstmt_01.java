...
public class JdbcPstmt_01 {
	...
	public void insertSangpum() {
		...
		// PreparedStatement은 미완의 sql문을 작성할 수 있다
		// 변수를 ?로 표시후 나중에 바인딩 시켜주면 된다
		String sql = "insert into sales values(seq_1.nextval,?,?,?,?,sysdate)";
	...
	// 수정...num를 인자로 받아서 데이타가 있으면 true,없으면 false반환하는 메소드
	public boolean inOneData(String num) {
		boolean b = false;
		Connection conn = db.getOracle();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public void updateSangpum() {
		...
		// 여기에서 inOneData()를 호출해서 그번호가 존재하는지 확인
		if (!this.inOneData(num)) {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			System.out.println("1.상품추가  2.상품전체출력  3.상품삭제  4상품수정 5.검색(이름일부) 9종료");
