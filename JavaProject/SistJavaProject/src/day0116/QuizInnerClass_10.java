...
class SawonOuter {
	...
	public void process() {
		...
		//데이타출력
		SudangInner inner = new SudangInner();
		inner.writeSawonData();
	}
	...
	class SudangInner {
		...
		public void writeSawonData() {
			...
			System.out.println(sawonName + "\t" + gibonPay + "\t" + familySu + "\t" + timeSu + "\t" + getFamilySudang()
					+ "\t" + getTimeSudang() + "\t" + getTex() + "\t" + getTotalPay());
		}
	}
}
...
public class QuizInnerClass_10 {
	...
	public static void main(String[] args) {
		...
		//방법1 : 내부클래스를 외부믈래스 통해서 생성후 메서드 호출
...
//		Output.writeSawonData();
		...
		//방법2 : 외부클래스의 멤버에서 내부클래스의 멤버를 호출후 메인에서는 외부클래스 생성 외부메서드 호출
		SawonOuter result = new SawonOuter();
		result.process();
